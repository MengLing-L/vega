...
fn test_map_partitions() -> Result<()> {
    ...
    let tmp: BigInteger256 = UniformRand::rand(&mut rng);
    let col = vec![tmp, tmp, tmp, tmp];
    let rdd = sc.make_rdd(col, 2);
    let partition_sums = rdd
        .map_partitions(Fn!(
            |iter: Box<dyn Iterator<Item = BigInteger256>>| 
            Box::new(std::iter::once(iter.sum::<BigInteger256>()))
                as Box<dyn Iterator<Item = BigInteger256>>
        ))
        .reduce(Fn!(
            |a : BigInteger256, b: BigInteger256| {
            let mut c = a.clone();
            c.add_nocarry(&b); 
            c
        }));
        //.collect()?;
    println!("partition_sums: {:?}", partition_sums);
...
#[test]
fn test_paring_map_partitions() -> Result<()> {
    let sc = CONTEXT.clone();
    let mut rng = ark_std::test_rng();
    let tmp: G1Projective = rng.gen();
    let mut a: G1Affine = tmp.into();
    let col = vec![a, a];
    let rdd = sc.make_rdd(col, 2);
    let partition_sums = rdd
        .map_partitions(Fn!(
            |iter: Box<dyn Iterator<Item = G1Affine>>| 
            Box::new(std::iter::once(iter.sum::<G1Affine>()))
                as Box<dyn Iterator<Item = G1Affine>>
        ))
        .reduce(Fn!(
            |a : G1Affine, b: G1Affine| a + b)).unwrap().unwrap();
        //.collect()?;
    println!("partition_sums: {:?}", partition_sums);
    assert_eq!(partition_sums, a + a);
    //assert_eq!(rdd.glom().collect()?, vec![vec![1, 2], vec![3, 4]]);
    Ok(())
}
...
fn test_aggregate() {
    ...
    let pairs = sc.make_rdd(
        vec![
            ("a".to_string(), Box::new(tmp)),
            ("b".to_string(), Box::new(tmp)),
            ("a".to_string(), Box::new(tmp)),
            ("c".to_string(), Box::new(tmp)),
            ("a".to_string(), Box::new(tmp)),
        ],
        2,
    );
    use std::collections::{HashMap, HashSet};
    type StringMap = HashMap<String, Box<BigInteger256>>;
    let empty_map = StringMap::new();
    let mut zero = BigInteger256::from(0u64);
    let merge_element = Fn!(|mut map: StringMap, pair: (String, Box<BigInteger256>)| {
        map.entry(pair.0).or_insert(Box::new(zero));
        //a.add_nocarry(&pair.1);
        map
    });
    let merge_maps = Fn!(|mut map1: StringMap, map2: StringMap| {
        for (key, value) in map2 {
            //let mut a: BigInteger256 = map1.entry(key).as_mut_ptr().or_insert(zero);
            map1.entry(key).or_insert(Box::new(zero));
            //a.add_nocarry(&value);
        }
        map1
    });
...
fn test_take() -> Result<()> {
    ...
    let mut rng = ark_std::test_rng();
    let tmp: BigInteger256 = UniformRand::rand(&mut rng);
    let col1 = vec![tmp, tmp, tmp];
    println!("col1: {:?}", col1);
    let col1_rdd = sc.make_rdd(col1, 4);

    let taken_1 = col1_rdd.take(1)?;
    println!("taken_1: {:?}", taken_1);
